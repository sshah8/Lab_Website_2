...
function viewStudentStats(id, toggle){
	console.log("reached viewStudentStats");
	if (toggle === 1){
...
function loadStatsPage(){
	console.log("reached fillWinners")
   ...
   for (var j = 0, col; col = row.cells[j]; j++) {
		 if (j === 1){
		 	opponent = row.cells[j].innerText;
		 }
		 if (j === 2){
			 row2_val = parseInt(row.cells[j].innerText);
		 }
		 if (j === 3){
			 row3_val = parseInt(row.cells[j].innerText);
		 }
		 if (j === 4){
			 if (row3_val > row2_val){
				 row.cells[j].innerText = opponent;
				 losses++;
			 }
			 else{
				 row.cells[j].innerText = "CU Boulder";
				 wins++;
			 	}
		 	}
   	}
